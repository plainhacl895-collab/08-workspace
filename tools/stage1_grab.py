...
START_CHROME_SCRIPT = SCRIPT_DIR / "start_chrome_worker.ps1"  # 从工作区加载
...
# 超时保护配置
OVERALL_TIMEOUT_SECONDS = 90 * 60  # 整体超时 90 分钟
DISTRICT_TIMEOUT_SECONDS = 15 * 60  # 单区超时 15 分钟
DISTRICT_RUN_TIMEOUT = 5400.0
DISTRICT_POLL_INTERVAL = 5.0
DISTRICT_STALL_TIMEOUT = 300.0

# 允许跳过的最大区数，超过则整体失败
MAX_SKIPPABLE_DISTRICTS = 0  # 一个区都不能少
...
def send_login_reminder() -> None:
    ...
    try:
        ...
        reminder_text = "房源抓取需要登录！请在专用 Chrome 调试窗口中扫码登录账号"
        url = f"https://api.telegram.org/bot{bot_token}/sendMessage"
        params = {"chat_id": chat_id, "text": reminder_text, "parse_mode": "HTML"}
        data = urllib.parse.urlencode(params).encode()
        req = urllib.request.Request(url, data=data, method="POST")
        req.add_header("Content-Type", "application/x-www-form-urlencoded")
        urllib.request.urlopen(req, timeout=10)
        log("阶段 1", "Telegram 登录提醒已发送")
    except Exception as e:
        log("阶段 1", f"发送提醒失败：{e}")
...
def ensure_base_page(driver: WebDriver) -> bool:
    ...
    last_state = ""
    reminder_sent = False
    reminder_waited = 30  # 30 seconds then send reminder
    reminder_start = None
    ...
    while time.time() < deadline:
        ...
        if state != last_state:
            if state == "login":
                ...
                if reminder_start is None:
                    reminder_start = time.time()
            else:
                log("阶段 1", "页面还在加载或切换中，继续等待列表出现...")
            last_state = state

        # Login reminder: send Telegram notification after 30 seconds
        if is_login_page and not reminder_sent:
            if reminder_start and (time.time() - reminder_start) >= reminder_waited:
                send_login_reminder()
                reminder_sent = True
...
def reset_collector_state(driver: WebDriver) -> None:
    """重置房源收集器状态，允许开始下一个区的抓取"""
    try:
        # 重新初始化收集器，而不是调用不存在的 resetState
        driver.execute_script("""
            window.__codexHouseCollector = undefined;
        """)
        time.sleep(0.5)
        # 重新安装收集器
        install_browser_collector(driver)
        log("阶段 1", "JavaScript 收集器已重置")
    except Exception as e:
        log("阶段 1", f"WARNING: 重置状态失败：{e}")
...
def main() -> int:
    ...
    log("阶段 1", f"超时保护：整体{OVERALL_TIMEOUT_SECONDS/60:.0f}分钟，单区{DISTRICT_TIMEOUT_SECONDS/60:.0f}分钟")
    ...
    if not check_debug_port():
        ...
        if not launch_debug_chrome(force=False):
            ...
            if not launch_debug_chrome(force=True):
                ...
                return 1
        log("阶段 1", "OK: 调试 Chrome 已启动，复用工作区 ChromeDebugProfile")
    ...
    try:
        ...
        for idx, district in enumerate(DISTRICTS, 1):
            # 【新增】检查整体超时
            elapsed_minutes = (time.time() - overall_start_time) / 60
            if elapsed_minutes > OVERALL_TIMEOUT_SECONDS / 60:
                log("阶段 1", f"WARNING: 整体超时 {OVERALL_TIMEOUT_SECONDS/60:.0f} 分钟，停止抓取后续区域")
                log("阶段 1", f"已抓取 {idx-1}/{len(DISTRICTS)} 个区，{len(all_houses)} 套房源")
                break  # 停止抓取，保留已有数据
            
            # 【新增】检查单区超时（更精确的控制）
            district_start_time = time.time()

            # 【修改】每个区开始前重新加载页面并重置状态
            log("阶段 1", f"重新加载页面，准备抓取{district['name']}区 ({idx}/{len(DISTRICTS)})")
            driver.get(BASE_URL)
            time.sleep(3)  # 等待页面加载
            install_browser_collector(driver)
            reset_collector_state(driver)
            ...
            try:
                ...
                district_elapsed = (time.time() - district_start_time) / 60
                log(
                    "阶段 1",
                ...
                try:
                    ...
                    chat_id = "8724466632"
                    bot_token = "8705450288:AAExGrG5ZIKt3wUnoAZ2Bll4W2StzDVCOYY"
                    total_so_far = sum(len([h for h in all_houses if h.get('district') == d['name']]) for d in DISTRICTS[:idx+1])
                    progress_text = f"📊 {district['name']} 区完成\n\n✅ 该区：{len(district_houses)} 套\n📈 累计：{total_so_far} 套\n⏳ 进度：{idx+1}/{len(DISTRICTS)} 区"
                    url = f"https://api.telegram.org/bot{bot_token}/sendMessage"
                    params = {"chat_id": chat_id, "text": progress_text, "parse_mode": "HTML"}
                    data = urllib.parse.urlencode(params).encode()
                    req = urllib.request.Request(url, data=data, method="POST")
                    urllib.request.urlopen(req, timeout=5)
                except Exception as e:
                    log("阶段 1", f"⚠️ 发送进度汇报失败：{e}")
            except Exception as exc:
                # 单区出错：重试 2 次，不跳过
                district_elapsed = (time.time() - district_start_time) / 60
                log("阶段 1", f"WARNING: {district['name']} 区出错：{exc} (耗时 {district_elapsed:.1f}分钟)")
                
                # 重试逻辑
                max_retry = 2
                for retry in range(1, max_retry + 1):
                    log("阶段 1", f"重试 {district['name']} 区 ({retry}/{max_retry})...")
                    ...
                    try:
                        ...
                        log(
                            "阶段 1",
                            f"{district['name']} 区重试成功：保留 {len(district_houses)} 套"
                            f"（原始 {len(district_houses_raw)} 套{duplicate_note}），"
                            f"页数 {result.get('page_count')}, 停止原因 {result.get('stop_reason')}",
                        )
                        break  # 重试成功，退出重试循环
                        ...
                        if retry < max_retry:
                            log("阶段 1", f"重试失败，10 秒后再次重试")
                            continue
                        else:
                            # 重试 2 次后仍失败，报错停止整个任务
                            log(
                                "阶段 1",
                                f"ERROR: {district['name']} 区重试{max_retry}次后仍失败，终止抓取",
                            )
                            raise RuntimeError(f"{district['name']} 区重试{max_retry}次后仍失败：{retry_exc}")

        overall_elapsed = (time.time() - overall_start_time) / 60
        log("阶段 1", "=" * 60)
        log("阶段 1", f"抓取完成，共 {len(all_houses)} 套房源")
        log("阶段 1", f"总耗时：{overall_elapsed:.1f}分钟")
...
if __name__ == "__main__":
    if os.environ.get("HOUSE_GRAB_PIPELINE_RUN") != "1" and "--direct" not in sys.argv[1:]:
        print("ERROR: 请运行 RUN_HOUSE_GRAB.cmd，不要单独运行 stage1_grab.py")
        sys.exit(2)
    sys.exit(main())