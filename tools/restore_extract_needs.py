...
path = r'C:\Users\Huawei\.openclaw\workspace-tuantuan\tools\property\extract_client_needs.py'

# 如果文件损坏，直接删除重新创建一个简化版本
os.remove(path)
...
def get_client_followups_from_cache(row: int, limit: int = 20) -> list[dict] | None:
    """从主缓存读取跟进记录"""
    main_cache_file = r'C:\\Users\\Huawei\\.openclaw\\workspace\\runtime\\tuantuan_cache.json'
...
def extract_client_needs(client_row: int, client_name: str = None) -> dict:
    ...
    if followups is None:
        ...
        try:
            subprocess.run(
                ['cmd', '/c', r'C:\\Users\\Huawei\\.openclaw\\workspace\\tools\\RUN_TUANTUAN_qidong.cmd', 'refresh'],
