...
def load_records_from_workbook() -> list[dict[str, Any]]:
    ...
    try:
        ws = workbook.worksheets[SHEET_INDEX - 1]
        records: list[dict[str, Any]] = []

        for row_index in range(START_ROW, ws.max_row + 1):
            block_code = str(ws.cell(row_index, 2).value or "").strip()
            community_formula = ws.cell(row_index, 3).value
            community_name, community_url = parse_hyperlink_formula(community_formula)
            district = str(ws.cell(row_index, 1).value or "").strip()
            house_id = str(ws.cell(row_index, HOUSE_ID_COLUMN).value or "").strip()
            detail_url = str(ws.cell(row_index, DETAIL_URL_COLUMN).value or "").strip()
            ...
            records.append(
                build_record(
                    ...
                    detail_url=detail_url,
                    source="workbook",
                    price=ws.cell(row_index, 6).value,
                    score=ws.cell(row_index, 9).value,
