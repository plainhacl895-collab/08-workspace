...
print("[INFO] 清理页面状态...")
cleanup_js = """
if (window.LOBSTER_CLEANUP) {
    clearInterval(window.LOBSTER_CLEANUP);
}
window.LOBSTER_DATA = null;
window.LOBSTER_CLEANUP = setInterval(() => {
    if (typeof isCollecting !== 'undefined') isCollecting = false;
}, 100);
console.log('[清理] 完成');
clearInterval(window.LOBSTER_CLEANUP);
"""
ws.send(json.dumps({"id": 1, "method": "Runtime.evaluate", "params": {"expression": cleanup_js}}))
time.sleep(2)
ws.recv()
...
# 执行
ws.send(json.dumps({"id": 3, "method": "Runtime.evaluate", "params": {"expression": js_code}}))
print("[OK] 脚本已发送")
...
while not done:
    try:
        ...
        if data.get("method") == "Console.messageAdded":
            ...
            if "龙虾" in text and "测试版" not in text:
                print(f"  {text}")
                sys.stdout.flush()
            ...
            if "抓取完成" in text and "准备就绪" in text:
                ...
                # 获取数据
                ws.send(json.dumps({"id": 4, "method": "Runtime.evaluate", "params": {"expression": "window.LOBSTER_DATA", "returnByValue": True}}))
